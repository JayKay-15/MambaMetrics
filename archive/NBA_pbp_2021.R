...
setwd("/Users/Jesse/Documents/MyStuff/NBA Analysis/PBP")

#### Part 1.1 - https://nbainrstats.netlify.app/post/adding-lineups-to-nba-play-by-play-data/ ####

game_logs <- game_logs(seasons = 2021)

games <- game_logs %>%
    select(idGame, slugTeam, slugOpponent, locationGame, dateGame) %>%
    mutate(slugTeamHome = ifelse(locationGame == "H", slugTeam, slugOpponent),
           slugTeamAway = ifelse(locationGame == "A", slugTeam, slugOpponent),
           gmMonth = month(ymd(dateGame))) %>%
    select(-c(slugTeam, slugOpponent, locationGame)) %>%
    distinct(idGame, .keep_all = TRUE)

# games_12 <- games %>% filter(gmMonth == 12)
# games_1 <- games %>% filter(gmMonth == 1)
# games_2 <- games %>% filter(gmMonth == 2)
# games_3 <- games %>% filter(gmMonth == 3)
# games_4 <- games %>% filter(gmMonth == 4)
# games_5 <- games %>% filter(gmMonth == 5)


# play_logs_12 <- play_by_play_v2(game_ids = unique(games_12$idGame), return_message = T)
# write.xlsx(play_logs_12, "/Users/Jesse/Documents/MyStuff/NBA Analysis/12.xlsx")
# 
# play_logs_1 <- play_by_play_v2(game_ids = unique(games_1$idGame), return_message = T)
# write.xlsx(play_logs_1, "/Users/Jesse/Documents/MyStuff/NBA Analysis/1.xlsx")

# play_logs_2 <- play_by_play_v2(game_ids = unique(games_2$idGame), return_message = T)
# write.xlsx(play_logs_2, "/Users/Jesse/Documents/MyStuff/NBA Analysis/2.xlsx")
# 
# play_logs_3 <- play_by_play_v2(game_ids = unique(games_3$idGame), return_message = T)
# write.xlsx(play_logs_3, "/Users/Jesse/Documents/MyStuff/NBA Analysis/3.xlsx")

# play_logs_4 <- play_by_play_v2(game_ids = unique(games_4$idGame), return_message = T)
# write.xlsx(play_logs_4, "/Users/Jesse/Documents/MyStuff/NBA Analysis/4.xlsx")
# 
# play_logs_5 <- play_by_play_v2(game_ids = unique(games_5$idGame), return_message = T)
# write.xlsx(play_logs_5, "/Users/Jesse/Documents/MyStuff/NBA Analysis/5.xlsx")

csv12 <- read_csv("/Users/Jesse/Documents/MyStuff/NBA Analysis/PBP/csv12.csv")
csv1 <- read_csv("/Users/Jesse/Documents/MyStuff/NBA Analysis/PBP/csv1.csv")
csv2 <- read_csv("/Users/Jesse/Documents/MyStuff/NBA Analysis/PBP/csv2.csv")
csv3 <- read_csv("/Users/Jesse/Documents/MyStuff/NBA Analysis/PBP/csv3.csv")
csv4 <- read_csv("/Users/Jesse/Documents/MyStuff/NBA Analysis/PBP/csv4.csv")
csv5 <- read_csv("/Users/Jesse/Documents/MyStuff/NBA Analysis/PBP/csv5.csv")

play_logs_all <- bind_rows(csv12, csv1, csv2, csv3, csv4, csv5)
...
lineup_game <- lineup_game %>%
    group_by(idGame, numberPeriod) %>%
    mutate(lineupHome = na_locf(lineupAfterHome, na.rm = FALSE),
           lineupAway = na_locf(lineupAfterAway, na.rm = FALSE),
           lineupHome = ifelse(is.na(lineupHome), na_locf(lineupBeforeHome, fromLast = TRUE, na.rm = FALSE), lineupHome),
           lineupAway = ifelse(is.na(lineupAway), na_locf(lineupBeforeAway, fromLast = TRUE, na.rm = FALSE), lineupAway),
...
# write.xlsx(lineup_game, "/Users/Jesse/Documents/MyStuff/NBA Analysis/lineup_game.xlsx")
write_csv(lineup_game, "/Users/Jesse/Documents/MyStuff/NBA Analysis/PBP/lineup_game.csv")

#### Part 2.1 - https://nbainrstats.netlify.app/post/extracting-nba-lineups-stats-from-play-by-play/ ####
...
# write.xlsx(lineup_stats, "/Users/Jesse/Documents/MyStuff/NBA Analysis/lineup_stats.xlsx")
write_csv(lineup_stats, "/Users/Jesse/Documents/MyStuff/NBA Analysis/PBP/lineup_stats.csv")

#### Part 3.1 - https://nbainrstats.netlify.app/post/analyzing-nba-lineups-stats/ ####
...
# write.xlsx(lineup_combinations, "/Users/Jesse/Documents/MyStuff/NBA Analysis/lineup_combinations.xlsx")
write_csv(lineup_combinations, "/Users/Jesse/Documents/MyStuff/NBA Analysis/PBP/lineup_combinations.csv")

#### Part 4.1 - https://nbainrstats.netlify.app/post/analyzing-nba-stats-with-players-on-off-the-court/ ####
...
plan(multisession)
shots_2021 <- teams_shots(all_active_teams = T,
                          seasons = 2021)
...
assist_pct_players <- lineup_game %>%
...
rebound_pct_lineups <- lineup_game %>%
...
rebound_pct_lineups %>%
    mutate(gotReb = case_when(!is.na(descriptionPlayHome) & lineupLocation == "Home" ~ "team",
                              !is.na(descriptionPlayVisitor) & lineupLocation == "Away" ~ "team",
                              TRUE ~ "opponent")) %>%
    count(lineup, gotReb) %>%
...
shots_lineups <- shots_2021 %>%
    mutate(timeQuarter = paste(str_pad(minutesRemaining, 2, side = "left", pad = 0),
                               str_pad(secondsRemaining, 2, side = "left", pad = 0), 
                               str_pad("00", 2, side = "left", pad = 0),sep = ":")) %>%
    left_join(pbp_shots) %>%
...
#### Part 1.2 - https://nbainrstats.netlify.app/post/calculating-offensive-rebound-in-the-nba-using-r/ ####
...
#### Part 2.2 - https://nbainrstats.netlify.app/post/adding-possessions-to-nba-play-by-play-data/ ####
...
#### Part 3.2 - https://nbainrstats.netlify.app/post/adding-possessions-to-nba-play-by-play-data-the-official-way/ ####
...
games_api <- logs_teams %>%
    distinct(idGame, dateGame) %>%
    left_join(logs_teams %>%
                  mutate(number_quarters = case_when(minutesTeam == 240 ~ "1, 2, 3, 4",
                                                     minutesTeam == 265 ~ "1, 2, 3, 4, 5",
                                                     minutesTeam == 290 ~ "1, 2, 3, 4, 5, 6",
                                                     minutesTeam == 315 ~ "1, 2, 3, 4, 5, 6, 7")) %>%
                  separate_rows(number_quarters, sep = ", ") %>%
                  distinct(idGame, number_quarters)) %>%
    mutate(dateGame = format(dateGame, "%Y%m%d"),
           game_link = glue("http://data.nba.net/prod/v1/{dateGame}/00{idGame}_pbp_{number_quarters}.json"))
...
function_pbp <- function(x){
    ...
    pbp_data %>%
        pluck("plays") %>%
        select(1:9) %>%
        mutate(idGame = as.integer(str_extract(x, "\\d{10}")),
               numberPeriod = parse_number(str_extract(x, "pbp_\\d\\.json"))) %>%
        as_tibble()
}

pbp_fromapi <- future_map_dfr(games_api$game_link, function_pbp)

pbp_fromapi <- read_csv("https://github.com/ramirobentes/NBAblog/releases/download/pbp-api-20-c32446f/data.csv",
                        col_types = cols(clock = "c")) %>%
    group_by(idGame) %>%
    mutate(numberEvent = row_number()) %>%
    ungroup()
...
slug_team_ids <- pbp_fromapi %>%
    mutate(slugTeam = str_extract(description, "^\\[[A-Z]{3}\\]"),
           slugTeam = str_extract(slugTeam, "[A-Z]{3}")) %>%
    select(description, slugTeam, teamId) %>%
    filter(!is.na(slugTeam)) %>%
    distinct(slugTeam, teamId)

game_teams <- pbp_fromapi %>%
    distinct(idGame, teamId) %>%
    filter(!is.na(teamId)) %>%
    group_by(idGame) %>%
    mutate(oppId = ifelse(teamId == max(teamId), min(teamId), max(teamId))) %>%
    ungroup()

api_poss <- pbp_fromapi %>%
    left_join(game_teams) %>%
    mutate(possession = case_when(eventMsgType %in% c(1, 2) ~ 1,
                                  eventMsgType == 3 & str_detect(description, "1 of 2|1 of 3") ~ 1,
                                  eventMsgType == 5 ~ 1,
                                  TRUE ~ 0)) %>%
    select(-c(isVideoAvailable))

jumpball_turnovers <- pbp_fromapi %>%
    filter((eventMsgType == 10 & str_detect(description, "Challenge")) | 
               # below: jump ball after stoppage where there's no turnover in the play-by-play description
               (eventMsgType == 10 & lag(eventMsgType) == 20 & clock == lag(clock) & !(lead(clock) == clock & lead(eventMsgType) == 5)) |
               (eventMsgType %in% c(4, 1, 3, 5) & !str_detect(description, "Technical|Flagrant|Clear Path"))) %>%
    group_by(idGame, numberPeriod) %>%
    filter(eventMsgType == 10 | lead(eventMsgType == 10)) %>%
    ungroup() %>%
    left_join(game_teams) %>%
    mutate(next_possession = case_when(eventMsgType == 4 ~ teamId,
                                       eventMsgType %in% c(1, 3, 5) ~ oppId,
                                       eventMsgType == 10 ~ teamId,
                                       TRUE ~ 0)) %>%
    group_by(idGame, numberPeriod) %>%
    filter(lag(next_possession) != next_possession) %>%
    ungroup()

api_poss <- api_poss %>%
    left_join(jumpball_turnovers %>%
                  transmute(idGame, numberEvent, possession_new = 1)) %>%
    mutate(teamId = ifelse(!is.na(possession_new), oppId, teamId),
           possession = ifelse(!is.na(possession_new), 1, possession)) %>%
    select(-possession_new)

new_possessions <- api_poss %>%
    filter(!eventMsgType %in% c(8, 9, 11, 13, 18, 20)) %>%   # substitution, timeout, end of period, , replay, stoppage
    group_by(idGame, numberPeriod) %>%
    filter(row_number() == max(row_number())) %>%
    ungroup() %>%
    select(idGame, numberPeriod, clock, eventMsgType, description, teamId, possession, numberEvent) %>%
    filter(clock != "00:00.0") %>%
    left_join(game_teams) %>%
    mutate(teamNewPoss = case_when(eventMsgType %in% c(4, 10) ~ teamId,   # rebound, jump ball
                                   eventMsgType %in% c(1, 3, 5, 6, 7) ~ oppId,   # turnover, fg, ft, foul, violations
                                   TRUE ~ teamId)) %>%
    transmute(idGame, numberPeriod, clock = "00:00.0", eventMsgType = 0, description = "last possession of quarter",
              teamId = teamNewPoss, possession = 1, numberEvent = numberEvent + 0.5)

api_poss <- api_poss %>%
    bind_rows(new_possessions) %>%
    arrange(idGame, numberEvent)

change_consec <- api_poss %>%
    filter(possession == 1) %>%
    group_by(idGame, numberPeriod) %>%
    filter(possession == lead(possession) & teamId == lead(teamId)) %>%
    ungroup() %>%
    mutate(possession_new = 0)

final_poss_api <- api_poss %>%
    left_join(change_consec %>%
                  select(idGame, numberEvent, possession_new)) %>%
    mutate(possession = ifelse(is.na(possession_new), possession, possession_new)) %>%
    select(-possession_new)

possessions_quarter <- final_poss_api %>%
    group_by(idGame, numberPeriod, teamId) %>%
    summarise(possessions = sum(possession)) %>%
    ungroup() %>%
    filter(!is.na(teamId)) %>%
    left_join(slug_team_ids) %>%
    select(idGame, numberPeriod, slugTeam, possessions)
...
points_quarter <- pbp_fromapi %>%
    group_by(idGame, numberPeriod) %>%
    filter(row_number() == max(row_number())) %>%
    group_by(idGame) %>%
    mutate(pointsHome = hTeamScore - lag(hTeamScore, default = 0),
           pointsAway = vTeamScore - lag(vTeamScore, default = 0)) %>%
    ungroup() %>%
    select(idGame, numberPeriod, pointsHome, pointsAway) %>%
    pivot_longer(cols = starts_with("points"),
                 names_to = "pointsLocation",
                 values_to = "points",
                 names_prefix = "points") %>%
...
compare_poss <- possessions_quarter %>%
    left_join(logs_teams %>%
                  distinct(idGame, slugTeam, slugOpponent, dateGame)) %>%
    left_join(points_quarter) %>%
    mutate(pts_per100 = round((points / possessions) * 100, 1),
...
compare_poss %>%
    count(poss_diff = possessions - possessions_nba) %>%
    mutate(poss_diff_pct = n / sum(n))
