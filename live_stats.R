...
lg_avg <- dplyr::tbl(DBI::dbConnect(RSQLite::SQLite(),
                 ...
                 "league_avg_current") %>% collect() %>% mutate(Date = as_date(Date, origin ="1970-01-01"))
    
sched <- nbastatR::current_schedule() %>% filter(dateGame == Sys.Date())
# sched <- nbastatR::current_schedule() %>% filter(dateGame == '2022-11-28')
sched[,c(29,24,16,8)]
...
### run live stats ----
scoreboard('2022-12-09')
team_stats(0022200382)
player_stats(0022200382)

# ---------------------------- archive

live_stats <- function(gm) {
